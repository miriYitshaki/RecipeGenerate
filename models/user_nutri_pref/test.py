...
from recipe_gen.language import START_INDEX, PAD_INDEX, TECHNIQUES_LIST, END_INDEX
        ...
        for i, batch in enumerate(tqdm(sampler.epoch_batches(), total=sampler.n_batches), 1):
            ...
            for ix in range(len(generated_strs)):
                ...
                prior_tech_attn_df.to_pickle(
                    os.path.join(trial_save_dir, 'prior_nutri_attention.pkl')
                )
...
'''
==== RUN CLUSTER
python -m recipe_gen.models.user_technique_pref.test --data-dir <DATA FOLDER> --model-path <MODEL PATH> --vocab-emb-size 300 --tech-emb-size 50 --calorie-emb-size 5 --nhid 256 --nlayers 2 --save-dir <OUTPUT FOLDER> --ingr-gru --ingr-emb --ingr-emb-size 10 --overwrite --batch-size 48
'''
