...
class PersonalTechEncoderDecoder(EncoderDecoder):
    """
...
class PersonalTechDecoder(Decoder):
...
def create_model(vocab_emb_dim, tech_emb_dim, calorie_emb_dim, hidden_size, n_layers,
    ...
    if decode_name:
        ...
        model = PersonalTechEncoderDecoder(encoder, decoder, name_decoder=name_decoder)
    else:
        model = PersonalTechEncoderDecoder(encoder, decoder)
    if use_cuda:
        model = model.cuda()
